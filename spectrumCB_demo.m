...
for i=1:length(data1(1,:))
    ...
    a = fft(data2(:,i));
    amp=abs(a(1:(N+1)/2)).^2; %take half of spectrum and square, beware of units here
    amp = amp / N.^2; % MATLAB normalization
    amp = amp .* 2; % lost variance
    amp = amp / df; % definition of the spectrum, this also changes the units
    A(:,i) = amp;
end
a = mean(A,2); %Power Spectral Density
